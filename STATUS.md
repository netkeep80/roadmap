...
- Last successful GitHub check: **2026-08-24T18:43:02.722Z**
- Latest observed GitHub change in snapshot: **2026-08-24T18:42:24Z**
- State hash (excluding check time): `c0cf7eefd6de438c9d13bb5b39bb2d51befe3e1a00745f42bb5c2068789b9ee2`

## Control-plane health

- ✅ Child backlink coverage: 25/25.
...
|---:|---|---|---|---|
| **P0** | [#3](https://github.com/netkeep80/roadmap/issues/3) MTS semantics/proof + aprover functional prover + AVM runtime + independent visual lane | `active` | 🟡 open | anum_docs#925/#926 → one aprover repin → exact Theory binding → aprover#181 → theorem/search/C1-C6; anum_docs#944 drives AST-free SyntaxAset convergence; mts_visual remains independent; AVM #174→#131 continues |
...
| **P1** | [`isocubic`](https://github.com/netkeep80/isocubic) | `active` | восстановить test-first product core и завершить dev-tool extraction boundary | #300 → #301 → #302/#303/#304 → #305 → #306/#307 | 13 issues / 1 PRs | [#5](https://github.com/netkeep80/roadmap/issues/5) |
...
| **P1** | [`PersistMemoryManager`](https://github.com/netkeep80/PersistMemoryManager) | `active` | закрыть consumer-shaped readiness для native persistent object storage pjson | #410/#415/#416/#426 → #421 | 16 issues / 6 PRs | [#2](https://github.com/netkeep80/roadmap/issues/2) |
| **P1** | [`pjson`](https://github.com/netkeep80/pjson) | `blocked` | довести pjson 1.0 до native PMM-backed JSON без private storage workaround | после PMM#421: #55 → #34, затем #35..#44 | 17 issues / 0 PRs | [#2](https://github.com/netkeep80/roadmap/issues/2) |
| **P1** | [`repo-guard`](https://github.com/netkeep80/repo-guard) | `active` | раскатывать consumer-driven governance baseline без speculative DSL growth | Tier A active repos use reproducible immutable-pinned policy | 7 issues / 0 PRs | [#6](https://github.com/netkeep80/roadmap/issues/6) |
| **P1** | [`roadmap`](https://github.com/netkeep80/roadmap) | `control-plane` | быть единой точкой входа для актуального состояния, зависимостей, решений и направления развития всего portfolio | ongoing: поддерживать live control loop и явно reconciliate detected semantic drift | 57 issues / 1 PRs | [#1](https://github.com/netkeep80/roadmap/issues/1), [#13](https://github.com/netkeep80/roadmap/issues/13), [#16](https://github.com/netkeep80/roadmap/issues/16) |
...
| **P2** | [`jsonRVM`](https://github.com/netkeep80/jsonRVM) | `oracle` | сохранять differential corpus/provenance до закрытия AVM migration | AVM#131 differential migration complete | 1 issues / 1 PRs | [#3](https://github.com/netkeep80/roadmap/issues/3), [#8](https://github.com/netkeep80/roadmap/issues/8) |
...
| **P2** | [`mts-genesis`](https://github.com/netkeep80/mts-genesis) | `research` | дать самодостаточное концептуальное объяснение общей идеи, не создавая второй нормативной МТС | formalizable claims route into explicit versioned anum_docs research beyond the current accepted production baseline | 2 issues / 0 PRs | [#9](https://github.com/netkeep80/roadmap/issues/9), [#11](https://github.com/netkeep80/roadmap/issues/11) |
...
**Objective:** поддерживать accepted MTS v0.11 как единственный current production/reference semantic+proof baseline; package/identity и SyntaxAset work развивается через явные bounded/versioned gates, а visualization package authority находится отдельно в mts_visual

**Next gate:** #925 portable theorem reuse + #926 untrusted producer facade завершить поверх accepted v0.11; #944 SyntaxAset architecture продолжает независимый S1+ migration path; #122/#123 остаются versioned research extensions; visual consumers repin independently
...
**Objective:** стать functional mathematical MTS prover: exact consumer accepted anum_docs proof semantics, trusted exact-Theory replay/approval, untrusted theorem/search/import tooling и независимая presentation lane через exact mts_visual consumer boundary

**Next gate:** anum_docs#925/#926 accepted → ONE exact @mts/core repin → trusted expected-Theory binding → #181 theorem record → theorem repository/index → deterministic untrusted search → C1-C6 corpus; #168/#183 source/visual migration proceeds independently without AST authority

**Depends on:** [`anum_docs`](https://github.com/netkeep80/anum_docs), [`mts_visual`](https://github.com/netkeep80/mts_visual)

- 🟡 [#153](https://github.com/netkeep80/aprover/issues/153) `open` — [Roadmap] `aprover` → mathematical MTS prover: trusted replay, exact Theory binding, theorem library + search
- 🟡 [#157](https://github.com/netkeep80/aprover/issues/157) `open` — [R3] Functional mathematical prover: theorem library → deterministic search → C1-C6 corpus
- 🟡 [#168](https://github.com/netkeep80/aprover/issues/168) `open` — [Visual migration] Перейти на standalone @mts/visual без расширения trusted proof boundary
- 🟡 [#181](https://github.com/netkeep80/aprover/issues/181) `open` — [R3.1] Versioned replayable theorem record без stored-truth authority
- 🟡 [#183](https://github.com/netkeep80/aprover/issues/183) `open` — [Architecture] Убрать canonical AST: direct Syntax Aset + Aset-native prover/UI
...
**Next gate:** #174 semantic migrator → #131 differential end-to-end; #169 remains Native JSON umbrella; MTS-dependent frontend exact-repins только явный current accepted anum_docs boundary, без local alternative semantics
...
|---|---|---:|---|---:|---:|
| [`anum_docs`](https://github.com/netkeep80/anum_docs) | `main` | no | 2026-08-24T18:38:28Z | 12 | 2 |
| [`aprover`](https://github.com/netkeep80/aprover) | `main` | no | 2026-08-24T15:58:25Z | 6 | 1 |
...
| [`mast-calculator`](https://github.com/netkeep80/mast-calculator) | `main` | no | 2026-08-24T18:23:52Z | 11 | 1 |
| [`PersistMemoryManager`](https://github.com/netkeep80/PersistMemoryManager) | `main` | no | 2026-08-24T17:47:37Z | 16 | 6 |
| [`pjson`](https://github.com/netkeep80/pjson) | `main` | no | 2026-08-09T09:56:53Z | 17 | 0 |
| [`repo-guard`](https://github.com/netkeep80/repo-guard) | `main` | no | 2026-08-24T12:00:46Z | 7 | 0 |
| [`roadmap`](https://github.com/netkeep80/roadmap) | `main` | no | 2026-08-24T18:42:24Z | 57 | 1 |
...
| [`jsonRVM`](https://github.com/netkeep80/jsonRVM) | `master` | no | 2026-08-09T09:59:38Z | 1 | 1 |
| [`meta_rm`](https://github.com/netkeep80/meta_rm) | `main` | no | 2026-08-09T09:58:55Z | 0 | 0 |
| [`mts-genesis`](https://github.com/netkeep80/mts-genesis) | `main` | no | 2026-08-09T09:59:08Z | 2 | 0 |
